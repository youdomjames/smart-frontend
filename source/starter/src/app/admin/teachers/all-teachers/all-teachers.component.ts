  ...
  implements OnInit
{
  ...
  viewTeacher(teacher){
    const dialogRef = this.dialog.open(ViewComponent, {width: "50%",
      data: {
        teacher: teacher
      }
    });
  ...
  addNew() {
    ...
    const dialogRef = this.dialog.open(FormDialogComponent, {width: "60%",
      data: {
  ...
  deleteItem(row) {
    ...
    const dialogRef = this.dialog.open(DeleteDialogComponent, { width: "20%",
      data: row,
      direction: tempDirection,
    });
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
